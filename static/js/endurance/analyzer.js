...
function analyzerRelayScoreScroll(direction){
 ...
 const step=56*3;
 scroller.scrollBy({left:(direction<0?-step:step),behavior:'smooth'});
 setTimeout(analyzerRelayScoreUpdateNavButtons,220);
}
function analyzerRelayScoreUpdateNavButtons(){
 const scroller=document.getElementById('analyzerRelayScoreXScroll');
 const left=document.getElementById('analyzerRelayScorePrev'),right=document.getElementById('analyzerRelayScoreNext');
 if(!scroller)return;
 const max=Math.max(0,scroller.scrollWidth-scroller.clientWidth),x=scroller.scrollLeft;
...
async function analyzerLoadRelayScores({force=false,background=false,structuralSignature=null,contextKey=null}={}){
 ...
 try{
  ...
  if(String(error?.message||error)==='SCORE_RELAIS_CONTEXT_CHANGED'){
   ...
   analyzerRelayScoreLastStatus={state:'error',done:Number(analyzerRelayScoreProgress.done)||0,total:Number(analyzerRelayScoreProgress.total)||drivers.length,team:String(analyzerRelayScoreProgress.team||''),phase:'error',updatedAt:Date.now(),durationMs:0,error:String(error?.message||error)};
...
function analyzerRenderRelayScoreTable(marketByScore){
 ...
 if(xscroll){
  ...
  requestAnimationFrame(()=>{const maxScroll=Math.max(0,xscroll.scrollWidth-xscroll.clientWidth);xscroll.scrollLeft=Math.min(Math.max(0,analyzerRelayScoreScrollLeft),maxScroll);analyzerRelayScoreUpdateNavButtons()});
 }
}
...
// V7.2.1790 — Position du chrono LIVE : même source de vérité que Focus Endurance.
// Le dernier tour piste valide de l'équipe suivie est comparé au dernier tour
// piste valide de chaque autre équipe, indépendamment du numéro de tour.
function analyzerRenderFollowedPerformance(followed){
 const chronoEl=document.getElementById('analyzerFollowedLastChrono');
 if(!chronoEl)return;
 const rank=followed&&typeof window.velocityLiveLastLapRanking==='function'?window.velocityLiveLastLapRanking(followed):null;
 const rankText=rank&&Number.isFinite(rank.rank)?`P${rank.rank}`:'P—';
 const lapText=rank?.lap||followed?.last||'—';
...
function analyzerRenderSpotterSync(){
 ...
 const cards=(spotter.queue||[]).map((item,index)=>{
  ...
  const teamKey=reserved?item.reservedTeam:item.lastTeam;const name=typeof spotterDisplayName==='function'?spotterDisplayName(teamKey||origin):(teamKey||origin);const teamNo=typeof spotterTeamNumber==='function'?spotterTeamNumber({team:teamKey,teamNumber:item.teamNumber}):'—';const status=reserved?'ATTRIBUÉ':'DISPONIBLE';
  return `<div class="analyzer-spotter-kart ${reserved?'reserved':'available'}"><span class="analyzer-spotter-position">${index+1}</span><span class="spotter-team-number">${analyzerEscape(teamNo)}</span><strong>${analyzerEscape(name||'—')}</strong><small>${analyzerEscape(item.kv||'—')} · ${status}</small><b>Score ${item.score??'—'} · Conf. ${item.confidence==null?'—':item.confidence+'%'}</b></div>`;
 }).join('');
 const maintenance=(spotter.maintenance||[]).map(item=>`<div class="analyzer-spotter-kart maintenance"><span class="spotter-team-number">${analyzerEscape(typeof spotterTeamNumber==='function'?spotterTeamNumber({team:item.lastTeam,teamNumber:item.teamNumber}):'—')}</span><strong>${analyzerEscape(typeof spotterDisplayName==='function'?spotterDisplayName(item.lastTeam||item.apexKart||'—'):(item.lastTeam||item.apexKart||'—'))}</strong><small>${analyzerEscape(item.kv||'—')} · ZONE MECA</small><b>Score ${item.score??'—'} · Conf. ${item.confidence==null?'—':item.confidence+'%'}</b></div>`).join('');
...
async function fetchAllApexTeamPits(rowId,sessionId,status,expectedPitCount=0){
 ...
 for(let wi=0;wi<windows.length;wi++){
  ...
  let parsed=[];
  for(let attempt=1;attempt<=maxAttempts;attempt++){
   try{parsed=parseApexPitData(await apexHistoryRequest(command),rowId)}
   catch(error){
    parsed=[];
    console.warn('[Velocity][Apex STATS PITS] requête en échec',{rowId,sessionId:sessionId||'live',count,attempt,error});
   }
   if(parsed.length)break;
   console.warn('[Velocity][Apex STATS PITS] réponse vide, nouvel essai',{rowId,sessionId:sessionId||'live',count,attempt,maxAttempts});
   if(attempt<maxAttempts)await apexHistorySleep(120*attempt);
  }
  if(!parsed.length)continue;
  latest=parsed;
 ...
 if(!latest.length)console.warn('[Velocity][Apex STATS PITS] aucun arrêt récupéré après retries',{rowId,sessionId:sessionId||'live'});
 return latest;
}
...
function raceSessionTab(){raceSessionShowPane('raceSessionLanding')}

let velocityRaceDraftMembers=[];let velocityRacePendingEdits={team:null,session:null};
...
function raceSessionDraftRole(i,role,checked){const m=velocityRaceDraftMembers[i];if(!m)return;const set=new Set(m.roles||[]);checked?set.add(role):set.delete(role);m.roles=[...set]}
async function raceSessionCreateTeamAndContinue(){const name=String(document.getElementById('raceCreateTeamName')?.value||'').trim();if(!name)return raceSessionFeedback("Saisissez le nom de l'équipe.",true);const members=velocityRaceDraftMembers.map(m=>({name:String(m.name||'').trim(),roles:m.roles||[]})).filter(m=>m.name);try{let r=await fetch('/api/teams',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});let data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Création impossible');velocityRaceTeamId=data.team.id;for(const m of members){r=await fetch(`/api/teams/${encodeURIComponent(data.team.id)}/members`,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(m)});const md=await r.json();if(!r.ok||!md.ok)throw new Error(md.error||`Ajout de ${m.name} impossible`)}await refreshRaceSessionManager();void velocityTeamBackupRefresh();raceSessionOpenCreateCourse()}catch(e){raceSessionFeedback(e.message||String(e),true)}}
function raceSessionOpenCreateCourse(){const team=currentRaceTeam();if(!team)return raceSessionFeedback('Sélectionnez une équipe.',true);const sel=document.getElementById('raceSessionCircuitSelect');if(sel){sel.innerHTML=(state.circuits||[]).map(c=>`<option value="${analyzerEscape(c.id)}"${String(c.id)===String(state.circuit_id)?' selected':''}>${analyzerEscape(c.name)}</option>`).join('');if(state.circuit_id)sel.value=String(state.circuit_id)}const tn=document.getElementById('raceSessionTeamNameCreate');if(tn)tn.textContent=team.name;const sn=document.getElementById('raceSessionNameCreate');if(sn&&!sn.value)sn.value=String(state.session_name||'').trim();renderRaceAssignmentSelectors();raceSessionShowPane('raceSessionCreatePane')}
function raceSessionToggleEdit(kind){const isTeam=kind==='team',valueEl=document.getElementById(isTeam?'raceSessionActiveTeam':'raceSessionActiveName'),edit=document.getElementById(isTeam?'raceEditTeamBtn':'raceEditSessionBtn'),ok=document.getElementById(isTeam?'raceEditTeamOk':'raceEditSessionOk');if(!valueEl||!edit||!ok)return;const current=valueEl.textContent||'';valueEl.innerHTML=`<input class="race-inline-edit-input" value="${analyzerEscape(current)}">`;edit.hidden=true;ok.hidden=false;ok.classList.add('pending');valueEl.querySelector('input')?.focus()}
function raceSessionConfirmEdit(kind){const isTeam=kind==='team',valueEl=document.getElementById(isTeam?'raceSessionActiveTeam':'raceSessionActiveName'),edit=document.getElementById(isTeam?'raceEditTeamBtn':'raceEditSessionBtn'),ok=document.getElementById(isTeam?'raceEditTeamOk':'raceEditSessionOk');const value=String(valueEl?.querySelector('input')?.value||'').trim();if(!value)return;velocityRacePendingEdits[isTeam?'team':'session']=value;valueEl.textContent=value;edit.hidden=false;ok.hidden=true;ok.classList.remove('pending')}
async function saveRaceSessionChanges(){const assignments={team_manager:[],spotter:[],pilot:[]};document.querySelectorAll('[data-active-role-member]:checked').forEach(el=>{const role=el.dataset.activeRoleMember;if(assignments[role])assignments[role].push(el.value)});try{const body={assignments};if(velocityRacePendingEdits.team)body.team_name=velocityRacePendingEdits.team;if(velocityRacePendingEdits.session)body.session_name=velocityRacePendingEdits.session;const r=await fetch('/api/race-session/update',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Mise à jour impossible');velocityRacePendingEdits={team:null,session:null};velocityRaceSession=data.session;window.velocityRaceSession=data.session;raceSessionRender(data.session);closeRaceSessionManager();showMode('analyzer')}catch(e){raceSessionFeedback(e.message||String(e),true)}}
...
function openRaceSessionManager(){document.getElementById('raceSessionModal')?.classList.add('show');raceSessionFeedback('');refreshRaceSessionManager().then(()=>raceSessionShowPane(velocityRaceSession?.status==='active'?'raceSessionActive':'raceSessionLanding'))}
...
function openRaceMemberDeleteModal(memberId,memberName){
 const modal=document.getElementById('raceMemberDeleteConfirm');if(!modal)return;
 modal.dataset.memberId=memberId||'';modal.dataset.memberName=memberName||'';
 const label=document.getElementById('raceMemberDeleteName');if(label)label.textContent=memberName||'ce membre';
 modal.hidden=false;
}
function closeRaceMemberDeleteModal(){const modal=document.getElementById('raceMemberDeleteConfirm');if(modal){modal.hidden=true;modal.dataset.memberId='';modal.dataset.memberName=''}}
async function confirmRaceMemberDelete(){
 const modal=document.getElementById('raceMemberDeleteConfirm');
 const id=modal?.dataset.memberId||'',name=modal?.dataset.memberName||'';
 const btn=document.getElementById('raceMemberDeleteConfirmBtn');if(btn)btn.disabled=true;
 try{await performDeleteRaceMember(id,name);closeRaceMemberDeleteModal()}
 catch(e){raceSessionFeedback(e.message||String(e),true)}
...
async function createRaceSession(){const team=currentRaceTeam();if(!team)return raceSessionFeedback('Sélectionnez une équipe.',true);const button=document.querySelector('.race-session-create');if(button)button.disabled=true;try{const circuitId=String(document.getElementById('raceSessionCircuitSelect')?.value||state.circuit_id||'').trim();if(!circuitId)throw new Error('Sélectionnez un circuit.');if(String(state.circuit_id||'')!==circuitId){const rc=await fetch('/api/circuit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({circuit_id:circuitId})});const dc=await rc.json();if(!rc.ok||!dc.ok)throw new Error(dc.error||'Changement de circuit impossible')}const body={name:document.getElementById('raceSessionNameCreate')?.value?.trim()||'',team_id:team.id,team_name:team.name,assignments:currentAssignmentPayload()};const r=await fetch('/api/race-session/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Création impossible');velocityRaceSession=data.session;window.velocityRaceSession=data.session;raceSessionRender(data.session);closeRaceSessionManager();showMode('analyzer')}catch(e){raceSessionFeedback(e.message||String(e),true)}finally{if(button)button.disabled=false}}
...
async function velocityRaceAccessCheck(){const access=window.VELOCITY_RACE_ACCESS||{};if(!access.token)return;try{const r=await fetch(`/api/race-access/${encodeURIComponent(access.token)}`,{cache:'no-store'});if(!r.ok){velocityRaceAccessEnded();return}const data=await r.json();if(!data.ok)velocityRaceAccessEnded();else {}}catch(_){}}
