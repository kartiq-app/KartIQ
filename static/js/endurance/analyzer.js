...
// Velocity V7.2.88 — TEAM MANAGEMENT + Session Course persistante
...
function renderRaceTeamEditor(){const box=document.getElementById('raceTeamEditor'),team=currentRaceTeam();if(!box)return;if(!team){box.innerHTML='<div class="analyzer-empty">Créez ou sélectionnez une Team.</div>';return}box.innerHTML=`<div class="race-team-title"><div><span>TEAM</span><h3>${analyzerEscape(team.name)}</h3></div><div class="race-team-title-actions"><b>${team.members?.length||0} membre(s)</b><button class="danger" type="button" onclick="deleteRaceTeam('${analyzerEscape(team.id)}','${analyzerEscape(team.name).replace(/'/g,'&#39;')}')">SUPPRIMER LA TEAM</button></div></div><div class="race-member-list">${(team.members||[]).map(m=>{const paired=(m.device_ids||[]).length>0;return `<article class="race-member-card"><div class="race-member-head"><div><strong>${analyzerEscape(m.name)}</strong><span class="${paired?'paired':'unpaired'}">${paired?'● APPAREIL ASSOCIÉ':'○ APPAREIL NON ASSOCIÉ'}</span></div><div class="race-member-head-actions"><button onclick="inviteRaceMember('${analyzerEscape(m.id)}','${analyzerEscape(m.name).replace(/'/g,'&#39;')}')">${paired?'RÉASSOCIER':'ASSOCIER UN APPAREIL'}</button><button class="danger" onclick="deleteRaceMember('${analyzerEscape(m.id)}','${analyzerEscape(m.name).replace(/'/g,'&#39;')}')">SUPPRIMER</button></div></div><div class="race-member-roles"><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="pilot" ${m.roles?.includes('pilot')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> PILOTE</label><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="spotter" ${m.roles?.includes('spotter')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> SPOTTER</label><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="team_manager" ${m.roles?.includes('team_manager')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> TEAM MANAGER</label></div></article>`}).join('')||'<div class="analyzer-empty">Aucun membre.</div>'}</div><div class="race-add-member"><input id="raceNewMemberName" placeholder="Nom du nouveau membre"><div><label><input class="race-new-role" type="checkbox" value="pilot" checked> Pilote</label><label><input class="race-new-role" type="checkbox" value="spotter"> Spotter</label><label><input class="race-new-role" type="checkbox" value="team_manager"> Team Manager</label></div><button onclick="addRaceMember()">+ AJOUTER LE MEMBRE</button></div>`}
async function deleteRaceTeam(teamId,teamName){if(!confirm(`Supprimer la Team ${teamName} ? Les membres, appareils associés et invitations seront supprimés.`))return;try{const r=await fetch(`/api/teams/${encodeURIComponent(teamId)}`,{method:'DELETE'}),data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Suppression impossible');if(String(velocityRaceTeamId)===String(teamId))velocityRaceTeamId='';await refreshRaceSessionManager();raceSessionFeedback(`Team ${teamName} supprimée.`)}catch(e){raceSessionFeedback(e.message||String(e),true)}}
async function deleteRaceMember(memberId,memberName){if(!confirm(`Supprimer ${memberName} de la Team ? Son appareil et ses invitations seront également dissociés.`))return;try{const r=await fetch(`/api/members/${encodeURIComponent(memberId)}`,{method:'DELETE'}),data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Suppression impossible');await refreshRaceSessionManager();raceSessionFeedback(`${memberName} supprimé de la Team.`)}catch(e){raceSessionFeedback(e.message||String(e),true)}}
function openRaceInviteShare(memberName,link){velocityInviteShare={link:String(link||''),memberName:String(memberName||''),token:String(link||'').split('/').pop()||''};const modal=document.getElementById('raceInviteShareModal');document.getElementById('raceInviteShareTitle').textContent=`Associer ${memberName}`;document.getElementById('raceInviteShareText').textContent=`Envoyez ce lien à ${memberName}. Il ouvre la page d’installation de Velocity et associe ensuite son appareil.`;document.getElementById('raceInviteShareLink').value=velocityInviteShare.link;document.getElementById('raceInviteQrWrap').hidden=true;document.getElementById('raceInviteQrImage').removeAttribute('src');if(modal)modal.hidden=false}
function closeRaceInviteShare(){const modal=document.getElementById('raceInviteShareModal');if(modal)modal.hidden=true}
async function copyRaceInviteLink(){const link=velocityInviteShare.link;if(!link)return;try{await navigator.clipboard.writeText(link);raceSessionFeedback(`Lien d’invitation ${velocityInviteShare.memberName} copié.`)}catch(_){const input=document.getElementById('raceInviteShareLink');input?.select();document.execCommand?.('copy')}}
async function shareRaceInviteLink(){const link=velocityInviteShare.link;if(!link)return;const title=`Velocity — invitation de ${velocityInviteShare.memberName}`;if(navigator.share){try{await navigator.share({title,text:'Installez Velocity et associez votre appareil.',url:link});return}catch(e){if(e?.name==='AbortError')return}}await copyRaceInviteLink()}
function toggleRaceInviteQr(){const wrap=document.getElementById('raceInviteQrWrap'),img=document.getElementById('raceInviteQrImage');if(!wrap||!img||!velocityInviteShare.token)return;wrap.hidden=!wrap.hidden;if(!wrap.hidden&&!img.getAttribute('src'))img.src=`/api/invite/${encodeURIComponent(velocityInviteShare.token)}/qr?t=${Date.now()}`}
