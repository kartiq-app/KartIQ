...
function analyzerRenderPitSimulator(){
 ...
 const pitDots=pitQueue.map(({driver},index)=>{const x=Math.max(18,136-index*25),classes=['pit-simulator-dot','pit','pit-queued'];if(driver.driver===state.followed_driver)classes.push('followed');if(analyzerMapIsHighlighted(driver,'pit',top5))classes.push('highlighted');if(analyzerMapHighlight!=='none'&&!analyzerMapIsHighlighted(driver,'pit',top5)&&driver.driver!==state.followed_driver)classes.push('dimmed');return `<g class="${classes.join(' ')}" transform="translate(${x} 150)"><title>${analyzerEscape(driver.driver)}</title><circle r="8.5"></circle><text y=".5">${analyzerEscape(analyzerSimulationKartLabel(driver))}</text></g>`}).join('');
 const pitLane=`<section class="map-pitlane-pane" aria-label="Voie des stands"><div class="map-pitlane-title">PIT LANE</div><svg viewBox="0 0 150 292" role="img" aria-label="Karts dans la voie des stands"><path class="map-pitlane-line" d="M12 150 H138"></path>${pitDots}</svg></section>`;
 if(!analyzerApexRaceIsActive()){analyzerTrackAnimationAnchors.clear();host.innerHTML=`<div class="map-stage"><section class="map-radar-pane"><svg viewBox="0 0 270 292" role="img" aria-label="Radar inactif">${radar}<text class="pit-simulator-center-title" x="135" y="184">MAP</text><text class="pit-simulator-center-value pit-simulator-center-value-idle" x="135" y="202">Pas de course en cours</text></svg></section>${pitLane}</div>`;return}
