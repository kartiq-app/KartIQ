...
function analyzerRelayMetrics(driver){
 ...
 const transitionScore=Number.isFinite(raw.correctedDelta)&&transitionValues.length?analyzerPercentileScore(raw.correctedDelta,transitionValues,{lowerIsBetter:true}):50;
...
function analyzerVelocityLabMarket(){
 return (state.drivers||[]).map(driver=>({driver,metrics:analyzerRelayMetrics(driver)})).filter(item=>item.metrics.laps>=3).sort((a,b)=>b.metrics.score-a.metrics.score).map((item,index)=>({...item,top:index+1}));
}
...
function analyzerVelocityLabSelectedItems(){
 ...
 return [...analyzerVelocityLabSelected].map(name=>drivers.get(name)).filter(Boolean).map(driver=>({driver,metrics:analyzerRelayMetrics(driver)})).filter(item=>item.metrics.laps>=3).sort((a,b)=>b.metrics.score-a.metrics.score);
}
...
function renderAnalyzer(){
 ...
 const marketByScore=all.map(x=>({...x,relayMetrics:analyzerRelayMetrics(x.driver)})).filter(x=>x.relayMetrics.laps>=3).sort((a,b)=>b.relayMetrics.score-a.relayMetrics.score).map((x,index)=>({...x,kartiqTop:index+1,relayRemaining:analyzerKartRelayRemaining(x.driver)}));
...
// Velocity V7.2.87 — TEAM MANAGEMENT + Session Course persistante
let velocityRaceSession=null,velocityRaceTeams=[],velocityRaceTeamId='',velocityRaceAccessPoll=null,velocityDevicePoll=null,velocityDeviceRole='';
...
async function createRaceTeamPrompt(){const name=prompt('Nom de la Team :','');if(!name?.trim())return;try{const r=await fetch('/api/teams',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name:name.trim()})});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Création impossible');velocityRaceTeamId=data.team.id;await refreshRaceSessionManager();raceSessionFeedback(`Team ${data.team.name} créée.`)}catch(e){raceSessionFeedback(e.message||String(e),true)}}
...
function renderRaceTeamEditor(){const box=document.getElementById('raceTeamEditor'),team=currentRaceTeam();if(!box)return;if(!team){box.innerHTML='<div class="analyzer-empty">Créez ou sélectionnez une Team.</div>';return}box.innerHTML=`<div class="race-team-title"><div><span>TEAM</span><h3>${analyzerEscape(team.name)}</h3></div><b>${team.members?.length||0} membre(s)</b></div><div class="race-member-list">${(team.members||[]).map(m=>{const paired=(m.device_ids||[]).length>0;return `<article class="race-member-card"><div class="race-member-head"><div><strong>${analyzerEscape(m.name)}</strong><span class="${paired?'paired':'unpaired'}">${paired?'● APPAREIL ASSOCIÉ':'○ APPAREIL NON ASSOCIÉ'}</span></div><button onclick="inviteRaceMember('${analyzerEscape(m.id)}','${analyzerEscape(m.name).replace(/'/g,'&#39;')}')">${paired?'RÉASSOCIER':'ASSOCIER UN APPAREIL'}</button></div><div class="race-member-roles"><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="pilot" ${m.roles?.includes('pilot')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> PILOTE</label><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="spotter" ${m.roles?.includes('spotter')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> SPOTTER</label><label><input data-member-role="${analyzerEscape(m.id)}" type="checkbox" value="team_manager" ${m.roles?.includes('team_manager')?'checked':''} onchange="updateRaceMemberRoles('${analyzerEscape(m.id)}')"> TEAM MANAGER</label></div></article>`}).join('')||'<div class="analyzer-empty">Aucun membre.</div>'}</div><div class="race-add-member"><input id="raceNewMemberName" placeholder="Nom du nouveau membre"><div><label><input class="race-new-role" type="checkbox" value="pilot" checked> Pilote</label><label><input class="race-new-role" type="checkbox" value="spotter"> Spotter</label><label><input class="race-new-role" type="checkbox" value="team_manager"> Team Manager</label></div><button onclick="addRaceMember()">+ AJOUTER LE MEMBRE</button></div>`}
async function inviteRaceMember(memberId,memberName){try{const r=await fetch(`/api/members/${encodeURIComponent(memberId)}/invite`,{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Invitation impossible');const text=`Velocity — invitation de ${memberName}`;if(navigator.share){try{await navigator.share({title:text,text:'Installez Velocity et associez votre appareil.',url:data.link});raceSessionFeedback(`Invitation envoyée à ${memberName}.`);return}catch(e){if(e?.name==='AbortError')return}}try{await navigator.clipboard.writeText(data.link);raceSessionFeedback(`Lien d’invitation ${memberName} copié.`)}catch(_){prompt('Copiez ce lien :',data.link)}}catch(e){raceSessionFeedback(e.message||String(e),true)}}
function renderRaceAssignmentSelectors(){const team=currentRaceTeam();const defs=[['raceAssignTM','team_manager'],['raceAssignSpotter','spotter'],['raceAssignPilot','pilot']];for(const [id,role] of defs){const sel=document.getElementById(id);if(!sel)continue;const current=velocityRaceSession?.assignments?.[role]||sel.value;const members=(team?.members||[]).filter(m=>(m.roles||[]).includes(role));sel.innerHTML='<option value="">Non affecté</option>'+members.map(m=>`<option value="${analyzerEscape(m.id)}"${String(m.id)===String(current)?' selected':''}>${analyzerEscape(m.name)}</option>`).join('')}const teamLabel=document.getElementById('raceSessionTeam');if(teamLabel)teamLabel.textContent=team?.name||'—';const circuit=document.getElementById('raceSessionCircuit');if(circuit)circuit.textContent=raceSessionCircuitName()}
function currentAssignmentPayload(){return {team_manager:document.getElementById('raceAssignTM')?.value||'',spotter:document.getElementById('raceAssignSpotter')?.value||'',pilot:document.getElementById('raceAssignPilot')?.value||''}}
...
function renderActiveRaceAssignments(){const box=document.getElementById('raceActiveAssignments'),team=currentRaceTeam();if(!box||!velocityRaceSession)return;const roles=[['team_manager','TEAM MANAGER'],['spotter','SPOTTER'],['pilot','PILOTE']];box.innerHTML=roles.map(([role,label])=>{const members=(team?.members||[]).filter(m=>(m.roles||[]).includes(role));const current=velocityRaceSession.assignments?.[role]||'';return `<label><span>${label}</span><select data-active-role="${role}"><option value="">Non affecté</option>${members.map(m=>`<option value="${analyzerEscape(m.id)}"${String(m.id)===String(current)?' selected':''}>${analyzerEscape(m.name)}</option>`).join('')}</select></label>`}).join('')}
async function createRaceSession(){const team=currentRaceTeam();if(!team)return raceSessionFeedback('Sélectionnez une Team.',true);const button=document.querySelector('.race-session-create');if(button)button.disabled=true;try{const body={name:document.getElementById('raceSessionName')?.value?.trim()||'',team_id:team.id,assignments:currentAssignmentPayload()};const r=await fetch('/api/race-session/create',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Création impossible');velocityRaceSession=data.session;raceSessionRender(data.session);raceSessionFeedback('Session active. Les membres affectés n’ont plus qu’à ouvrir Velocity.')}catch(e){raceSessionFeedback(e.message||String(e),true)}finally{if(button)button.disabled=false}}
async function saveRaceAssignments(){const assignments={};document.querySelectorAll('[data-active-role]').forEach(s=>assignments[s.dataset.activeRole]=s.value||'');try{const r=await fetch('/api/race-session/assignments',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify({assignments})});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Mise à jour impossible');velocityRaceSession=data.session;raceSessionRender(data.session);raceSessionFeedback('Rôles actifs appliqués. Les appareils se mettent à jour automatiquement.')}catch(e){raceSessionFeedback(e.message||String(e),true)}}
async function endRaceSession(){if(!confirm('Terminer la session Velocity ? Les rôles Pilote et Spotter seront immédiatement fermés.'))return;try{const r=await fetch('/api/race-session/end',{method:'POST',headers:{'Content-Type':'application/json'},body:'{}'});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Arrêt impossible');velocityRaceSession=null;raceSessionRender(null);raceSessionFeedback('Session terminée. La Team reste enregistrée pour les prochaines courses.')}catch(e){raceSessionFeedback(e.message||String(e),true)}}
async function claimVelocityInvite(){const token=String(window.VELOCITY_INVITE_TOKEN||'');if(!token||token==='expired')return;const button=document.getElementById('velocityInviteClaim');if(button)button.disabled=true;const id=velocityEnsureDeviceId();try{const r=await fetch(`/api/invite/${encodeURIComponent(token)}/claim`,{method:'POST',headers:{'Content-Type':'application/json','X-Velocity-Device':id},body:JSON.stringify({device_id:id,device_name:navigator.userAgent.includes('iPhone')?'iPhone':navigator.userAgent.includes('Android')?'Android':'Appareil Velocity'})});const data=await r.json();if(!r.ok||!data.ok)throw new Error(data.error||'Association impossible');try{localStorage.setItem(VELOCITY_DEVICE_KEY,data.device_id)}catch(_){}document.getElementById('velocityInviteStatus').textContent=`✓ Appareil associé à ${data.member.name}. Installez maintenant Velocity sur l’écran d’accueil puis ouvrez l’icône.`;if(button){button.textContent='APPAREIL ASSOCIÉ';button.disabled=true}}catch(e){document.getElementById('velocityInviteStatus').textContent=e.message||String(e);if(button)button.disabled=false}}
async function velocityInviteBootstrap(){const token=String(window.VELOCITY_INVITE_TOKEN||'');if(!token)return;document.body.classList.add('velocity-invite-mode');const o=document.getElementById('velocityInviteOverlay');if(o)o.hidden=false;if(token==='expired'){document.getElementById('velocityInviteTitle').textContent='Invitation expirée';document.getElementById('velocityInviteText').textContent='Demandez au Team Manager de générer une nouvelle invitation.';document.getElementById('velocityInviteClaim').hidden=true;return}try{const r=await fetch(`/api/invite/${encodeURIComponent(token)}`,{cache:'no-store'}),data=await r.json();if(!data.ok)throw new Error();const i=data.invite;document.getElementById('velocityInviteTitle').textContent=`Bienvenue ${i.member_name} !`;document.getElementById('velocityInviteText').innerHTML=`Vous avez été invité à rejoindre <b>${analyzerEscape(i.team_name)}</b>.`;document.getElementById('velocityInviteRoles').innerHTML=(i.roles||[]).map(r=>`<span>${raceRoleIcon(r)} ${raceRoleLabel(r)}</span>`).join('')}catch(_){document.getElementById('velocityInviteText').textContent='Invitation indisponible.'}}
function velocityResetRoleUI(){document.body.classList.remove('race-role-access','race-role-spotter','race-role-pilot','velocity-device-waiting-mode');document.getElementById('velocityDeviceWaiting').hidden=true;try{document.getElementById('enduranceFocus')?.classList.remove('show')}catch(_){}}
function velocityApplyDeviceRole(data){const role=String(data?.role||'');if(role===velocityDeviceRole&&role)return;velocityDeviceRole=role;velocityResetRoleUI();if(!data?.paired)return;if(!role){if((data.authorized_roles||[]).includes('team_manager')){return}document.body.classList.add('velocity-device-waiting-mode');const w=document.getElementById('velocityDeviceWaiting');if(w)w.hidden=false;document.getElementById('velocityDeviceWaitingName').textContent=data.device?.member_name||'Membre associé';document.getElementById('velocityDeviceWaitingTeam').textContent=data.device?.team_name?`${data.device.team_name} — aucune session active pour vous.`:'Aucune session active.';return}if(role==='team_manager'){showMode('analyzer');return}document.body.classList.add('race-role-access',`race-role-${role}`);if(role==='spotter')showMode('spotter');else if(role==='pilot'){showMode('endurance');setTimeout(()=>openEnduranceFocus(),150)}}
async function velocityDeviceSessionCheck(){if(window.VELOCITY_INVITE_TOKEN)return;const id=velocityDeviceId();try{const r=await fetch('/api/device/session',{cache:'no-store',headers:id?{'X-Velocity-Device':id}:{}}),data=await r.json();if(data.ok&&data.paired){if(data.device?.id&&!id)try{localStorage.setItem(VELOCITY_DEVICE_KEY,data.device.id)}catch(_){}velocityApplyDeviceRole(data)}}catch(_){}}
function velocityDeviceBootstrap(){if(window.VELOCITY_INVITE_TOKEN)return;setTimeout(velocityDeviceSessionCheck,700);velocityDevicePoll=setInterval(velocityDeviceSessionCheck,3000)}
...
function velocityRaceAccessBootstrap(){const access=window.VELOCITY_RACE_ACCESS||{},role=String(access.role||'');if(role==='expired'){document.body.classList.add('race-role-access');velocityRaceAccessEnded();return}if(!['spotter','pilot'].includes(role))return;document.body.classList.add('race-role-access',`race-role-${role}`);const launch=()=>{if(!state?.circuit_id){setTimeout(launch,250);return}if(role==='spotter')showMode('spotter');else{showMode('endurance');setTimeout(()=>openEnduranceFocus(),180)}};launch();velocityRaceAccessCheck();velocityRaceAccessPoll=setInterval(velocityRaceAccessCheck,3000)}
document.addEventListener('DOMContentLoaded',()=>{velocityInviteBootstrap();velocityRaceAccessBootstrap();velocityDeviceBootstrap()});
window.openRaceSessionManager=openRaceSessionManager;window.closeRaceSessionManager=closeRaceSessionManager;window.raceSessionTab=raceSessionTab;window.selectRaceTeam=selectRaceTeam;window.createRaceTeamPrompt=createRaceTeamPrompt;window.addRaceMember=addRaceMember;window.updateRaceMemberRoles=updateRaceMemberRoles;window.inviteRaceMember=inviteRaceMember;window.createRaceSession=createRaceSession;window.saveRaceAssignments=saveRaceAssignments;window.endRaceSession=endRaceSession;window.claimVelocityInvite=claimVelocityInvite;
