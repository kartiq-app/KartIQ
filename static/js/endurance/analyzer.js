...
/* Velocity V7.2.54 — Mode Test Endurance longue durée */
window.velocityEnduranceTest={active:false,timer:null,backup:null,startedAt:0,simulatedMs:0,durationMs:0,speed:60,teams:40,laps:0,updates:0,stops:0,reconnectAttempts:0,reconnectSuccess:0,errors:[],log:[],drivers:[]};
...
function analyzerTestLog(message,type='info'){const t=window.velocityEnduranceTest,entry={at:new Date().toISOString(),simulatedMs:t.simulatedMs,message:String(message),type};t.log.unshift(entry);if(t.log.length>200)t.log.length=200;const host=document.getElementById('analyzerTestLog');if(host)host.innerHTML=t.log.slice(0,80).map(x=>`<div class="${x.type}"><time>${new Date(x.at).toLocaleTimeString('fr-FR')}</time>${analyzerEscape(x.message)}</div>`).join('')}
