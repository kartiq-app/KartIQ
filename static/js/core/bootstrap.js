...
if('serviceWorker' in navigator){window.addEventListener('load',()=>{
 ...
 (async()=>{try{await navigator.serviceWorker.register('/static/sw.js?v=7.2.176',{updateViaCache:'none'});const reg=await navigator.serviceWorker.getRegistration('/static/sw.js');reg?.update().catch(()=>{})}catch(err){console.warn('Service worker',err)}})();
})}
