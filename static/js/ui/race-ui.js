...
function render(){
 ...
 const sortedPenalties=[...(state.comment_penalties||[])].sort((a,b)=>String(b.time||b.at||'').localeCompare(String(a.time||a.at||''))); const penaltyMarkup=sortedPenalties.length?sortedPenalties.map(p=>`<div class="penalty-row sprint-comment-penalty" title="${escapePenaltyHtml(fullPenaltyText(p))}"><span class="penalty-portrait-full">${escapePenaltyHtml(fullPenaltyText(p))}</span><span class="penalty-landscape-compact">${escapePenaltyHtml(compactPenaltyText(p))}</span></div>`).join(''):'<div class="empty">Aucune pénalité</div>';
