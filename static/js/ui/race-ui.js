...
let circuitChangeInFlight=false;
let pendingCircuitId='';
...
function render(){
 ...
 const newCircuitSignature=state.circuits.map(c=>c.id+'|'+c.name).join('§');if(newCircuitSignature!==circuitSignature){circuitSignature=newCircuitSignature;circuitSelectElement.innerHTML='<option value="" selected disabled>Sélectionnez votre circuit</option>'+state.circuits.map(c=>`<option value="${c.id}">${c.name}</option>`).join('')}circuitSelectElement.value=pendingCircuitId||state.circuit_id||'';
 if(circuitSelectElement)circuitSelectElement.disabled=circuitChangeInFlight;
 ...
 const f=state.followed||{};qPos.textContent=f.pos?'P'+f.pos:'—';setDriverName(qName,f.driver||'—');qDelta.textContent=qualificationDeltaFor(f);qDelta.classList.toggle('delta-good',qualificationDeltaIsLeader(f));updateRemainingDisplay();qLapsRemaining.textContent=raceUsesLapTarget()?'':(state.apex_laps_remaining||'—');const qReference=qualificationReferenceFor(f);qGapWith.innerHTML=gapWithMarkup('vs',qReference?.driver||'—');
 const eQPos=document.getElementById('eQPos'),eQName=document.getElementById('eQName'),eQDelta=document.getElementById('eQDelta'),eQRemaining=document.getElementById('eQRemaining'),eQLapsRemaining=document.getElementById('eQLapsRemaining'),eQGapWith=document.getElementById('eQGapWith');if(eQPos)eQPos.textContent=f.pos?'P'+f.pos:'—';if(eQName)setDriverName(eQName,f.driver||'—');if(eQDelta){const enduranceDelta=sprintDeltaFor(f);eQDelta.textContent=enduranceDelta;eQDelta.classList.toggle('sprint-leader-delta',Number(f.pos)===1&&enduranceDelta!=='--');eQDelta.classList.toggle('sprint-followed-delta',Number(f.pos)>1&&enduranceDelta!=='--')}const enduranceLapMode=raceUsesLapTarget();const enduranceRemainingMs=enduranceLapMode?null:liveRemainingMilliseconds();if(eQRemaining){eQRemaining.textContent=enduranceLapMode?formatRaceLapProgress():formatMainRemainingDisplay(enduranceRemainingMs,state.time_remaining||'—');eQRemaining.classList.toggle('time-critical',!enduranceLapMode&&Number.isFinite(enduranceRemainingMs)&&enduranceRemainingMs<120000)}if(eQLapsRemaining)eQLapsRemaining.textContent=enduranceLapMode?'':(state.apex_laps_remaining||'—');if(eQGapWith)eQGapWith.innerHTML=gapWithMarkup('vs',qReference?.driver||'—');
 setDriverName(sName,f.driver||'—');sPos.textContent=f.pos?'P'+f.pos:'—';const sprintDelta=sprintDeltaFor(f);const sprintReference=sprintReferenceFor(f);sDeltaValue.textContent=sprintDelta;sDeltaValue.classList.toggle('sprint-leader-delta',Number(f.pos)===1&&sprintDelta!=='--');sDeltaValue.classList.toggle('sprint-followed-delta',Number(f.pos)>1&&sprintDelta!=='--');const sprintLapMode=raceUsesLapTarget();const sprintRemainingMs=sprintLapMode?null:liveRemainingMilliseconds();sRemaining.textContent=sprintLapMode?formatRaceLapProgress():formatMainRemainingDisplay(sprintRemainingMs,state.time_remaining||'—');sRemaining.classList.toggle('time-critical',!sprintLapMode&&Number.isFinite(sprintRemainingMs)&&sprintRemainingMs<120000);sLapsRemaining.textContent=sprintLapMode?'':(state.apex_laps_remaining||'—');sGapWith.innerHTML=gapWithMarkup('vs',sprintReference?.driver||'—');const fastest=sprintFastestLastLapForFollowed(f)||{};const fastestDriver=fastest.driver||'—';const fastestLap=fastest.last||'—';sFastestName.textContent='🔥 '+fastestDriver;sFastestTime.textContent=fastestLap;const fastestLapSeconds=parseLapTime(fastestLap);const sessionBestSeconds=absoluteSessionBestSeconds();const isAbsoluteSessionBest=Number.isFinite(fastestLapSeconds)&&Number.isFinite(sessionBestSeconds)&&Math.abs(fastestLapSeconds-sessionBestSeconds)<0.0005;const improvedPersonalBest=Boolean(fastest.last_improved_personal_best);sFastestTime.classList.toggle('fastest-session-best',isAbsoluteSessionBest);sFastestTime.classList.toggle('fastest-lap-green',!isAbsoluteSessionBest&&improvedPersonalBest);sFastestTime.classList.toggle('fastest-lap-orange',!isAbsoluteSessionBest&&!improvedPersonalBest);
...
async function changeCircuit(event){
 const selectedValue=String(event?.target?.value||circuitSelectElement?.value||'').trim();
 if(!selectedValue||circuitChangeInFlight)return;
 const previousCircuitId=String(state?.circuit_id||'');
 circuitChangeInFlight=true;
 pendingCircuitId=selectedValue;
 if(circuitSelectElement){circuitSelectElement.value=selectedValue;circuitSelectElement.disabled=true}
 document.getElementById('homeCircuit')?.classList.remove('needs-selection','circuit-error');
 try{
  ...
  state={...(state||{}),circuit_id:selectedValue,drivers:[],followed_driver:'',followed:null,penalties:[],quick_change:[],qualif_crossing:null,generic_alert:null,time_remaining:'—',apex_laps_remaining:'—',session_best:{driver:'—',lap:'—'},fastest_last_lap:{driver:'—',lap:'—'}};
  render();
  const response=await fetch('/api/circuit',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({circuit_id:selectedValue})});
  if(!response.ok)throw new Error(`Sélection du circuit impossible (${response.status})`);
  await load();
  ...
  pendingCircuitId='';
  state={...(state||{}),circuit_id:previousCircuitId};
  document.getElementById('homeCircuit')?.classList.add('circuit-error');
  render();
 }finally{
  circuitChangeInFlight=false;
  pendingCircuitId='';
...
// Le onchange HTML garantit le fonctionnement même si un autre module échoue pendant son initialisation.
