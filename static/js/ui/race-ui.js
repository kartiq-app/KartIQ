...
function render(){
 ...
 rows('enduranceTable',d=>{const isFollowed=d.driver===state.followed_driver;const enduranceDelta=isFollowed?sprintDeltaFor(d):(d.gap||'—');const enduranceDeltaClass=isFollowed?(Number(d.pos)===1?' endurance-delta-leader':' endurance-delta-followed'):'';return `<td class="pos endurance-position-cell"><span class="endurance-position-wrap"><span class="endurance-position-number${Number(d.pos)===1?' leader-pos':''}">${d.pos}</span>${endurancePitBadge(d)}</span></td><td class="name">${formatDriverName(d.driver)}</td><td>${d.apex}</td><td>${d.laps}</td><td>${d.pit_stops ?? '—'}</td><td class="endurance-stand-time-column">${d.pit_timer||'—'}</td><td class="${lapTimeClass(d,d.last,'last')}">${d.last}</td><td class="${lapTimeClass(d,d.best,'best')}">${d.best}</td><td class="endurance-delta-cell${enduranceDeltaClass}">${enduranceDelta}</td><td class="red">${d.penalty||'—'}</td>`});
