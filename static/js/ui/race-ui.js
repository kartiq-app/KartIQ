...
function render(){
 ...
 setDriverName(sName,f.driver||'—');sPos.textContent=f.pos?'P'+f.pos:'—';const sprintDelta=sprintDeltaFor(f);const sprintReference=sprintReferenceFor(f);sDeltaValue.textContent=sprintDelta;sDeltaValue.classList.toggle('sprint-leader-delta',Number(f.pos)===1&&sprintDelta!=='--');sDeltaValue.classList.toggle('sprint-followed-delta',Number(f.pos)>1&&sprintDelta!=='--');const sprintRemainingMs=liveRemainingMilliseconds();sRemaining.textContent=sprintRemainingMs===null?(state.time_remaining||'—'):formatRemainingMilliseconds(sprintRemainingMs);sRemaining.classList.toggle('time-critical',Number.isFinite(sprintRemainingMs)&&sprintRemainingMs<120000);sLapsRemaining.textContent=state.apex_laps_remaining||'—';sGapWith.innerHTML=gapWithMarkup('vs',sprintReference?.driver||'—');const fastest=state.fastest_last_lap||{};const fastestFallback=(state.drivers||[]).filter(d=>Number.isFinite(parseLapTime(d.last))).sort((a,b)=>parseLapTime(a.last)-parseLapTime(b.last))[0]||{};const fastestDriver=fastest.driver&&fastest.driver!=='—'?fastest.driver:(fastestFallback.driver||'—');const fastestLap=fastest.lap&&fastest.lap!=='—'?fastest.lap:(fastestFallback.last||'—');sFastestName.textContent='🔥 '+fastestDriver;sFastestTime.textContent=fastestLap;const fastestLapSeconds=parseLapTime(fastestLap);const sessionBestSeconds=absoluteSessionBestSeconds();const isAbsoluteSessionBest=Number.isFinite(fastestLapSeconds)&&Number.isFinite(sessionBestSeconds)&&Math.abs(fastestLapSeconds-sessionBestSeconds)<0.0005;sFastestTime.classList.toggle('fastest-session-best',isAbsoluteSessionBest);sFastestTime.classList.toggle('fastest-lap-green',!isAbsoluteSessionBest);
